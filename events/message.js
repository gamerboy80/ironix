module.exports = async (client, message) => {
	...
	if (
		message.content.toLowerCase() == "??resetprefix" ||
		message.content.toLowerCase() == prefix + "resetprefix"
	) {
	...
	if (!Array.isArray(client.xpblocked.get(message.guild.id))) {
		...
		iss = !client.xpblocked.get(message.guild.id).includes(message.channel.id);
		iss2 = !client.xpblocked.get(message.guild.id).includes(message.author.id);
	...
	if (
		...
		if (rankData.xp > needed) {
			...
			if (client.levelUpChannel.get(message.guild.id) == undefined) {
				...
				if (client.levelUpChannel.get(message.guild.id) == "dms") {
					...
					if (
						!client.levelUpChannel.get(message.guild.id) == "disabled" &&
		...
		if (
			client.leveluproles.get(message.guild.id)[rankData.level] != undefined
		) {
			try {
				message.guild.members.cache
					.get(message.author.id)
					.roles.add(client.leveluproles.get(message.guild.id)[rankData.level]);
			} catch {}
		}
	}
...
var needed = (Math.round(Math.pow(rankData.level, 2) / 10) + 2) * 10;
  ...
  client.rankData.set(message.author.id, rankData); */
	...
	if (client.aliases.get(message.author.id)) {
		if (client.aliases.get(message.author.id)[command]) {
			command = client.aliases.get(message.author.id)[command];
		}
	}
	...
	if (
		!client.ableUsers.get(message.author.id) &&
		message.content != prefix + "agree"
	) {
		message.channel.send({
			embed: {
				color: 0x51c878,
				description:
					"For using Ironix, you need to agree to our [Privacy Policy](https://ironix.emeraldteam.ga/privacy). Send " +
					prefix +
					"agree to agree to it and start using Ironix.",
				footer: {
					text: `Requested by ${message.author.username}#${message.author.discriminator} (${message.author.id})`,
					icon_url: message.author.displayAvatarURL(),
				},
	...
	if (!Array.from(client.usersOnCountdown.keys()).includes(message.author.id)) {
		...
		if (Array.isArray(cmd.neededPerms)) {
			cmd.neededPerms.forEach((perm) => {
				if (
					message.guild.members.cache.get(client.user.id).hasPermission(perm)
				) {
					try {
					...
					message.channel.send({
						embed: {
							color: 0xc85151,
							description:
								"Missing permissions: ``" + cmd.neededPerms.join(", ") + "``.",
							footer: {
								text: `Requested by ${message.author.username}#${message.author.discriminator} (${message.author.id})`,
								icon_url: message.author.displayAvatarURL(),
							},
