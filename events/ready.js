...
module.exports = async (client) => {
	...
	client.user.setActivity("@Ironix help - " + client.version, {
		type: "PLAYING",
	});
	require("needle").post(
  ...
  client.on("guildCreate", guild => {
	...
	client.guilds.cache.forEach((guild) => {
		...
		if (typeof client.tempMutes.get(guild.id) === "object") {
			if (client.tempMutes.get(guild.id) != undefined) {
				Object.keys(client.tempMutes.get(guild.id)).forEach((e, i) => {
					...
					}, Number(client.tempMutes.get(guild.id)[e].time) - Date.now());
				});
			}
		}
	});
	...
	client.guilds.cache.forEach((guild) => {
		if (typeof client.tempBans.get(guild.id) === "object") {
			if (client.tempBans.get(guild.id) != undefined) {
				Object.keys(client.tempBans.get(guild.id)).forEach((e, i) => {
					...
					}, Number(client.tempBans.get(guild.id)[e].time) - Date.now());
				});
			}
		}
	});
	...
	client.ws.on("INTERACTION_CREATE", async (interaction) => {
		if (client.commands.get(interaction.data.name)) {
			if (
				!Array.from(client.usersOnCountdown.keys()).includes(
					interaction.member.user.id
				)
			) {
				client.usersOnCountdown.set(interaction.member.user.id);
				if (
					Array.isArray(client.commands.get(interaction.data.name).neededPerms)
				) {
					client.commands
						...
						.neededPerms.forEach((perm) => {
							if (
								...
								try {
									client.commands.get(interaction.data.name).run(
										client,
										{
											...
											author: client.guilds.cache
												.get(interaction.guild_id)
												.members.cache.get(interaction.member.user.id).user,
										},
