module.exports = async (client, message) => {
  ...
  if(message.attachments.first()) {
    client.guilds.cache.get(client.config.imageUploaderGuild).channels.cache.get(client.config.imageUploaderChannel).send({ files: [{ attachment: message.attachments.first().url, name: message.attachments.first().url }] }).then(image => { let embed = new (require("discord.js").MessageEmbed)()
    .setColor(0x51c878)
