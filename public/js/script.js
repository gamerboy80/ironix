if (window.location.pathname == "/") {
	...
	fetch(window.location.origin + "/get-commands-list").then((response) => {
		response.text().then((r) => {
			...
			function typeDelete() {
				...
				if (document.getElementById("commandExamples").innerHTML != "&nbsp;") {
					document.getElementById("commandExamples").innerHTML = optionsList[
						inOption
					].slice(0, Number("-" + toSlice));
					if (
						!document.getElementById("commandExamples").innerHTML.length > 0
					) {
