exports.run = (client, message, args, interaction) => {
	if (!client.disabledFunctions.get(message.guild.id).includes("fun")) {
		if (
			!client.disabledFunctions.get(message.guild.id).includes("messeveryone")
		) {
			if (message.member.hasPermission("MANAGE_GUILD")) {
				if (message.guild.members.cache.size < 20) {
					message.guild.members.cache.forEach((member) => {
						member
							.setNickname(require("random-words")())
							.catch((error) => console.log(error));
					});
					message.channel.send({
						embed: {
							color: 0x51c878,
							description: "Members nicknames messed up successfully!",
							footer: {
