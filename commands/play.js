...
if(!Array.isArray(client.queue[message.guild.id])) {
  ...
  if(!message.guild.members.cache.get(client.user.id).voice.channel) {
    if(video.vc) {
    video.vc = await message.guild.channels.cache.get(video.vc.channel.id).join().catch(console.log);
  } else {
    if(message.member.voice.channel) {
      video.vc = video.vc = await message.member.voice.channel.join().catch(console.log);
    } else {
      message.channel.send({
          embed: {
            color: 0xc85151,
