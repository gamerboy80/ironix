	...
	if (!client.disabledFunctions.get(message.guild.id).includes("moderation")) {
		if (!client.disabledFunctions.get(message.guild.id).includes("mute")) {
			...
			if (message.member.hasPermission("KICK_MEMBERS")) {
				if (
					message.guild.roles.cache.find((r) => r.name == "Muted-IX") == null
				) {
					message.guild.roles
						.create({
							data: {
						...
						.then((role) => {
							...
							muteRole.setPosition(
								message.guild.roles.cache.find((r) => r.name == "Ironix")
									.position - 1
							);
			...
			if (true) {
				if (args[2]) {
					client.users.fetch(getIdFromMention(args[0])).then((user) => {
						...
						function isMutable() {
							if (
								message.guild.members.cache.get(user.id).roles.highest ==
								"@everyone"
							) {
								return true;
							} else {
						...
						if (isMutable()) {
							...
							if (
								client.mutes.get(message.guild.id).includes(user.id) &&
								message.guild.members.cache
									.get(user.id)
									.roles.cache.has(muteRole.id)
							) {
								if (message.author.id != client.user.id)
									message.channel.send({
										embed: {
												...
												if (dm != undefined) {
													dm.send(
														"**" +
															...
															new Date(
																Number(
																	client.tempMutes.get(message.guild.id)[
																		user.id
																	].time
																)
															) +
