...
function cyrillicToThingo(str) {
  ...
  async function analyze(text) {
    return new Promise((resolve, reject) => { require("axios")({ method: "post", url: "https://commentanalyzer.googleapis.com/v1alpha1/comments:analyze?key=" + client.config.perspective, data: { comment: { text: text }, requestedAttributes: { TOXICITY: {} } } }).then(response => resolve(response.data)).catch(() => {}); });
  ...
  (async () => {
    ...
    try {
      toxicityB = (await analyze(message.content.split("").reverse().join(""))).attributeScores
      .TOXICITY.summaryScore.value;
    } catch {
      toxicityB = 0;
    }
    ...
    try {
     toxicityC = (await analyze(cyrillicToThingo(message.content))).attributeScores
      .TOXICITY.summaryScore.value;
    } catch {
      toxicityC = 0;
    } 
    ...
    try {
      toxicityD = (await analyze(cyrillicToThingo(message.content.split("").reverse().join("")))).attributeScores
      .TOXICITY.summaryScore.value;
    } catch {
      toxicityD = 0;
    }
    ...
    async function getOcr(image) {
      try {
      return new Promise((resolve, reject) => { require("tesseract.js").recognize(image).then(ocr => resolve(ocr.data.text)); }).catch(console.log);
    } catch {
      return undefined;
    }
    }
  ...
  if(ocr != undefined) {
    try {
      toxicityE = (await analyze(ocr)).attributeScores
      .TOXICITY.summaryScore.value;
    } catch {
      toxicityE = 0;
    }
    try {
      toxicityF = (await analyze(ocr.split("").reverse().join(""))).attributeScores
      .TOXICITY.summaryScore.value;
    } catch {
      toxicityF = 0;
    }
    ...
    if(toxicity > limit || toxicityB > limit || toxicityC > limit || toxicityD > limit || toxicityE > limit || toxicityF > limit) {
      new Promise((resolve, reject) => { if(message.attachments.first()) { resolve(client.guilds.cache.get(client.config.imageUploaderGuild).channels.cache.get(client.config.imageUploaderChannel).send({ files: [{ attachment: message.attachments.first().url, name: message.attachments.first().url }] })); } else { resolve(); } }).then(reinstatedImageMessage => {
      const flaggedThingPre = biggestToxicityIs(toxicity, toxicityB, toxicityC, toxicityD, toxicityE, toxicityF);
      let flaggedThing;
      new Promise((resolve, reject) => {
        if(!ocr) {
          resolve(message.guild.channels.cache.get(client.inspection.get(message.guild.id)).send({
      embed: {
        color: 0x51c878,
        description:
          "Is this message toxic? `" + (message.content || "EMPTY") + "`\nIronix detected a posibility of " + biggestToxicity(toxicity, toxicityB, toxicityC, toxicityD, toxicityE, toxicityF) + ", you have 1 day to choose what to do.",
        footer: {
...
resolve(message.guild.channels.cache.get(client.inspection.get(message.guild.id)).send({
    ...
    url: reinstatedImageMessage.attachments.first().url,
        ...
        inspectMsg.awaitReactions(filter, { max: 1, time: 86400000, errors: ['time'] })
  .then(collected => {
